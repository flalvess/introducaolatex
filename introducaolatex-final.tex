...
\setotherlanguages{french,english,german,russian,greek,syriac,hebrew}
...
\newfontfamily\arabicfont[Script=Arabic,Numbers=Arabic]{Adobe Arabic}
...
%\setcapindent{0pt}
%\usepackage{mathtools} % só utilizar se inserir matemática (substitui amsmath)

\usepackage[style=historian,citestyle=authoryear,natbib=true,doi=false,backend=bibtex,dashed=false]{biblatex} %dashed=false nao funciona com style=historian
\usepackage{csquotes}
\DefineBibliographyStrings{brazil}{namedash={---------},volumeof={de},url={Disponível em: }} %para partes de livros de muitos volumes "Vol. 5 de" etc.
%\DefineBibliographyStrings{english}{%
%backrefpage={cited on p\adddot},
%backrefpages={cited on pp\adddot}
%} % se opção do biblatex for backref=true, e a língua do documento
\addbibresource{latex.bib} %para biber, necessário adicionar extensão .bib
...
Se você quiser criar sua própria lista de elementos não alinhados com o texto (\textit{floats}), separados de figuras e tabelas (por exemplo, uma ``Lista de Esculturas''), você pode usar o pacote \texttt{trivfloat}:

\begin{verbatim}
\usepackage{trivfloat}
\trivfloat{esculturas} % cria nova lista com nome: ``esculturas''
...

\begin{document}
\renewcommand{\listesculturaname}{Lista de Esculturas} % muda o nome da lista 
...
\begin{escultura}
\centering
\includegraphics{escultura-romana}
\caption{Escultura de Júlio César.}
\end{escultura}
\end{verbatim}
...
%No entanto, por puro diletantismo, este capítulo provê uma brevíssima introdução a esse tema.
%
%\input{matematica}
...
\begin{verbatim}
\setotherlanguage{hebrew} % para o pacote polyglossia, ou \setmainlangauge{hebrew} se hebraico for a língua principal.
\newfontfamily\hebrewfont[Script=Hebrew]{Adobe Hebrew}
...
...
Fonte: http://he.wikipedia.org/wiki/עברית
\end{hebrew}
\end{center}
