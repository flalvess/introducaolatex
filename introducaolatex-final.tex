...
%\setmainfont[Numbers=OldStyle]{SabonNext}
...
\title{\fontsize{50}{60}\selectfont \textcolor{YellowOrange}{Uma introdução ao \rmfamily\LaTeX}}
\subtitle{\textcolor{Dandelion}{\addfontfeatures{Style=TitlingCaps} \huge UM SISTEMA DE PREPARAÇÃO DE TEXTOS}}
%\extratitle{\phantom{XXX}\vspace{12cm} \raggedleft \Huge \emph{Uma Introdução ao} \LaTeX}
...
\begin{titlepage}
\phantom{XXX}
\vspace{10cm} \raggedleft \Huge \emph{Uma Introdução ao} \LaTeX
\end{titlepage}
...
\begin{quote}
A prática de fazer as coisas por si próprio já se difundiu amplamente há bastante tempo, mas os resultados são frequentemente duvidosos porque os tipógrafos leigos não veem o que está errado e não conseguem perceber o que é importante. Assim, fica-se acostumado a uma tipografia pobre e incorreta. [\ldots ] Ora, pode-se objetar que a tipografia depende do gosto. Se isso dissesse respeito somente a decoração, talvez deixássemos o argumento passar; entretanto, uma vez que a tipografia concerne precipuamente a informação, erros podem não somente irritar, como também até causar danos \parencite{willberg2000}.
\end{quote}
...
Numa obra-prima tipográfica, a assinatura do artista e eliminada. O que alguns podem elogiar como estilos pessoais são, na realidade, pequenas e vazias peculiaridades, frequentemente danosas, que se disfarçam de inovações. [\ldots] Tipografia pessoal é tipografia deficiente. Só iniciantes e bobos insistirão em usá-la \parencite[p. 25--26]{tschichold2007forma}.
\end{quote}
...
\begin{table}
\caption{Cruzes do pacote \texttt{bbding} \label{tab:bbding-cruzes}}
\begin{tabular}{lllll}
\Cross{}\verb+\Cross+& \CrossOpenShadow{} \verb+\CrossOpenShadow+ & \PlusOutline{} \verb+\PlusOutline+ & \CrossBoldOutline{}  \verb+\CrossBoldOutline+ & \CrossOutline{} \verb+\CrossOutline+\\
\PlusThinCenterOpen{} \verb+\PlusThinCenterOpen+ & \CrossClowerTips{} \verb+\CrossClowerTips+ & \Plus{} \verb+\Plus+ & \CrossMaltese{} \verb+\CrossMaltese+ & \PlusCenterOpen{} \verb+\PlusCenterOpen+\\
\end{tabular}
\end{table}
...
\phantom{}
\vfill
O tipo usado foi \textit{Jannon Text Moderne}, uma recriação dos tipos de Jean Jannon do século \textsc{XVII}, para texto, e \textsf{Profile}, de Martin Wenzel (1999), para títulos. A classe utilizada foi \textsf{\KOMAScript{}}, de Frank Neukam, Markus Kohm \& Axel Kielhorn.
