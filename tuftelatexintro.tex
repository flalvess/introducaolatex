\documentclass[a4paper,nols,bidi,nohyper,nobib]{tufte-book}
\usepackage{amssymb}
\usepackage{fontspec}
...
\usepackage[style=authoryear,dashed=false,backend=bibtex,doi=false,isbn=false,natbib=true]{biblatex}
...
%
%\AtEveryBibitem{\clearfield{month}} --> para limpar o mês da bibliografia, se necessário.
...
\fontspec{San Marco LT Std}{Ad maiorem Dei gloriam}
\end{center}
\end{fullwidth}
...
\begin{verbatim}
\usepackage[marginal]{footmisc} % opção "marginal" coloca os números da nota de rodapé para fora da margem, à la Bringhurst
\end{verbatim}
...
http://he.wikipedia.org/wiki/עברית
\end{hebrew}
\end{center}
...
{\chines 早安} & Zao an! & ``Bom dia!" em chinês\\

{\coreano 안녕하세요}   &  An-nyeong-ha-se-yo & ``Como vai?"/"Bom dia!" em coreano.
...
\phantom{}
\vfill
O tipo usado foi \textit{Jannon Text Moderne}, uma recriação dos tipos de Jean Jannon do século \textsc{xviii}, para texto, e \textsf{Profile}, de Martin Wenzel (1999), para títulos. A classe utilizada foi \emph{Tufte-LateX}.
