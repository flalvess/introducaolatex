\documentclass[a4paper,nols,bidi]{tufte-book}
\usepackage{amssymb}
\usepackage{fontspec}
...
Os \textit{ambientes} têm um papel semelhante a comandos, mas geralmente afetam só uma parte específica do documento. 
...
Como a rigor não existe interface gráfica para um arquivo .tex (que é de texto simples), alguns caracteres são usados para os comandos:
...
Algumas outras classes são: \texttt{book} (livro), \texttt{report}, \texttt{abnt} (formato ABNT), \texttt{beamer} (para apresentações de slides). Outras opções podem ser vistas na tabela \ref{tab:classes}.
...
\verb+\textsf{...}+ & \verb+{\sffamily}+ & \textsf{fonte sem serifas}\\
\verb+\texttt{...}+ & \verb+{\ttfamily}+ & \texttt{fonte com largura fixa}\\
...
São inseridas com o comando: \verb+\footnote{...}+.\footnote{Neste documento, todas as notas de rodapé são convertidas para notas na margem. Isso pode ser feito com as classes Tufte-\LaTeX{} ou o pacote \texttt{footmisc}.}
...
Se você quiser inserir o número do lado direito no começo da página (uma prática usual por aqui), o jeito mais fácil é pôr \verb+\pagestyle{myheadings}+ depois de \verb+\begin{document}+ e \verb+\thispagestyle{myheadings}+ depois do começo de cada capítulo. %O pacote \texttt{abntex} já faz tudo isso automaticamente.
...
Para escrever em chinês, japonês ou coreano, utilizamos o pacote \texttt{xeCJK} e o comando \verb+\setCJKmainfont{nome da fonte}+. Uma solu;áo para usar os três sistemas é:\footnote{A solução para um texto com os três sistemas (chinês, japonês e coreano) foi copiada de: \href{http://tex.stackexchange.com}{http://tex.stackexchange.com/questions/45331/combining-chinese-japanese-and-korean-text-with-xecjk}}
\label{japones}
\medskip
...
Outra solução, para textos curtos, é definir a fonte de forma simples:
...
Colocando entre chaves para o texto definido: \verb+{\japones}+
...
\verb+\begin{framed} ... \end{framed}+ & \begin{framed}Bordas ao redor do texto\end{framed}\\
\verb+\begin{shaded}... \end{shaded}+ & \begin{shaded} Texto com sombra de fundo \end{shaded}\\
\verb+\begin{snugshade} ... \end{snugshade}+ &  \colorbox{Gray}{Texto com sombra de fundo mais justa.}\\
\verb+\fcolorbox{Magenta}{Gray}{...}+ & \fcolorbox{Magenta}{Gray}{Define a cor da borda e a cor do fundo.} \\
\verb+\colorbox{Gray}{...}+ & \colorbox{Gray}{Um sombreado só do tamanho do texto.}\\
\verb+\framebox{...}+  & \framebox{Uma caixa ao redor do texto do tamanho do parágrafo.}\\
\verb+\fbox{XXX}+  & \fbox{XXX}\\
\end{tabular}
}
...
\begin{minipage}{1.0\marginparwidth}
\begin{mdframed}[backgroundcolor=lightgray]
...
\phantom{}
\vfill
Este texto foi composto e adaptado (uma tradução livre com acréscimos) de: \url{http://en.wikibooks.org/wiki/LaTeX/}. Tipologia: Bulmer. A classe utilizada foi \textit{Tufte-\LaTeX}.
